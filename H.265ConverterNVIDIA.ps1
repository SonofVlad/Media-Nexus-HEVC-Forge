...
<#
H.265ConverterNVIDIA.ps1  (Windows PowerShell 5.1 compatible)

Version 1.0 – Original logic
Version 1.1 – Added +genpts and warning-only decode handling
Version 1.2 – Capture ffmpeg stderr to temp files so PowerShell does not wrap
                native stderr in NativeCommandError records during decode validation
Version 1.3 – Added real failed-file backoff markers and excluded internal RAW folders
Version 1.4 – Added NVIDIA NVENC encoding tuned for archival compression
...
Other features:
- Local temporary encoding
- Live console dashboard (YES)
- Stable file check (default 60s)
- HEVC NVENC (hevc_nvenc) with auto-deinterlace when field_order != progressive (bwdif)
- P7 slowest/highest-quality preset, full-resolution two-pass, AQ, and lookahead enabled
- Encode -> validate -> copy to destination as .part -> rename to final
...
param(
    ...
    # HEVC NVENC constant quality: lower = higher quality/larger file.
    # 25 is the tested starting point for a balance of quality and file-size reduction.
    [ValidateRange(10,40)]
    [int]$NvencQuality,
...
# Verify NVENC encoder exists
$encodersText = & $FFmpegExe -hide_banner -encoders 2>$null | Out-String
if ($encodersText -notmatch "hevc_nvenc") {
    throw "FFmpeg does not report hevc_nvenc. Ensure the NVIDIA driver is installed and you have an FFmpeg build with NVENC support."
}
...
Log ("NvencQuality:   {0}" -f $NvencQuality)
Log "NVENC preset:    p7 (slowest/highest quality)"
Log "NVENC multipass: fullres"
Log "NVENC tuning:    VBR constant-quality, AQ, lookahead 32, 4 B-frames"
...
$workerScript = {
    ...
    function Decode-TestInner([string]$ffmpegExe, [string]$p) {
        $stderrFile = Join-Path $env:TEMP ("ffmpeg_decode_test_{0}.log" -f ([guid]::NewGuid().ToString("N")))
        try {
            if (Test-Path -LiteralPath $stderrFile) {
                Remove-Item -LiteralPath $stderrFile -Force -ErrorAction SilentlyContinue
            ...
            if (Test-IsIgnorableDecodeWarningInner $text) {
                return @{ Ok = $true; Text = $text; WarningOnly = $true }
            }
    ...
    try {
        $rel = Get-RelInner $ProcessRoot $SrcFull
        if (-not $rel) { throw "Could not compute relative path." }
        $result.RelPath = $rel
        Set-Status "START" $rel ""

        $srcItem = Get-Item -LiteralPath $SrcFull -ErrorAction Stop
        $srcSize = $srcItem.Length
        $result.SrcSizeMB = [Math]::Round($srcSize / 1MB, 1)
        ...
        $destFinal = Join-Path $dstDir ($baseName + "." + $OutputContainer)
        $result.OutputFinal = $destFinal

        if ($SkipIfOutputExists -and (Test-Path -LiteralPath $destFinal)) {
            $result.Status = "SKIP"
            $result.Note = "Output already exists"
            Set-Status "SKIP" $rel $result.Note
            return [pscustomobject]$result
        }

        # field_order (for deinterlace decision)
        $fieldOrder = & $FFprobeExe -v error -select_streams v:0 -show_entries stream=field_order -of default=nw=1:nk=1 $SrcFull 2>$null
        if (-not $fieldOrder) { $fieldOrder = "unknown" }
        $fieldOrder = $fieldOrder.Trim().ToLowerInvariant()
        $result.FieldOrder = $fieldOrder
        $needsDeint = ($fieldOrder -ne "progressive")
        $result.Deint = $needsDeint

        if ($needsDeint) {
            Set-Status "ENCODING" $rel "Deint=bwdif"
        } else {
            Set-Status "ENCODING" $rel "Deint=none"
        }

        # Local temp output (GUID names avoid extension weirdness)
        $tempDir = Join-Path $LocalTempRoot $env:COMPUTERNAME
        $null = New-Item -ItemType Directory -Force -Path $tempDir

        $tempOut = Join-Path $tempDir ("job_{0}.{1}.tmp" -f $JobId, $OutputContainer)
        $tempFinalLocal = Join-Path $tempDir ("job_{0}.{1}" -f $JobId, $OutputContainer)

        if (Test-Path -LiteralPath $tempOut) { Remove-Item -LiteralPath $tempOut -Force -ErrorAction SilentlyContinue }
        if (Test-Path -LiteralPath $tempFinalLocal) { Remove-Item -LiteralPath $tempFinalLocal -Force -ErrorAction SilentlyContinue }

        # Force muxer so ffmpeg never "guesses"
        $forcedMux = @()
        if ($OutputContainer -eq "mkv") { $forcedMux = @("-f","matroska") }
        elseif ($OutputContainer -eq "mp4") { $forcedMux = @("-f","mp4") }
        ...
        if ($needsDeint) {
            $baseArgs += @("-vf", "bwdif=mode=send_field:parity=auto:deint=all")
        }

        $tryCopySubs = ($OutputContainer -eq "mkv")

        # Attempt 1
        if ($tryCopySubs) {
            $args1 = $baseArgs + @("-c:a","copy","-c:s","copy","-map_metadata","0") + $forcedMux + @($tempOut)
        } else {
            $args1 = $baseArgs + @("-c:a","copy","-sn","-map_metadata","0") + $forcedMux + @($tempOut)
        }

        $stderr1File = Join-Path $env:TEMP ("ffmpeg_encode1_{0}.log" -f ([guid]::NewGuid().ToString("N")))
        $stderr2File = Join-Path $env:TEMP ("ffmpeg_encode2_{0}.log" -f ([guid]::NewGuid().ToString("N")))
        $stderr1 = ""
        $stderr2 = ""

        try {
            & $FFmpegExe @args1 2>$stderr1File
            $exit1 = $LASTEXITCODE
            if (Test-Path -LiteralPath $stderr1File) {
                $stderr1 = (Get-Content -LiteralPath $stderr1File -Raw -ErrorAction SilentlyContinue | Out-String)
            }

            if ($exit1 -ne 0) {
                if (Test-Path -LiteralPath $tempOut) { Remove-Item -LiteralPath $tempOut -Force -ErrorAction SilentlyContinue }

                # Attempt 2: always no subtitles
                $args2 = $baseArgs + @("-c:a","copy","-sn","-map_metadata","0") + $forcedMux + @($tempOut)
                & $FFmpegExe @args2 2>$stderr2File
                $exit2 = $LASTEXITCODE
                if (Test-Path -LiteralPath $stderr2File) {
                    $stderr2 = (Get-Content -LiteralPath $stderr2File -Raw -ErrorAction SilentlyContinue | Out-String)
                }

                if ($exit2 -ne 0 -or -not (Test-Path -LiteralPath $tempOut)) {
                    $result.Status = "FAIL"
                    $result.Note = "FFmpeg encode failed"
                    $result.ErrText = ("--- Attempt1 ---`r`n{0}`r`n--- Attempt2 ---`r`n{1}" -f $stderr1.Trim(), $stderr2.Trim()).Trim()
                    Set-Status "FAIL" $rel $result.Note
                    return [pscustomobject]$result
                } else {
                    $result.Note = "Subtitles skipped (retry succeeded)."
        ...
        # Copy to destination as .part then rename to final
...
function Render-Dashboard {
    ...
    try {
        ...
        Clear-Host
        Write-Host ("[{0}] Active: {1}/{2} | NVENC CQ={3} | Queue: {4}" -f (Get-Date -Format "HH:mm:ss"), $running.Count, $Parallel, $NvencQuality, $queueCount)
